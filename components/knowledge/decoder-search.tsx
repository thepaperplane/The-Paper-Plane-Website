...
> = {
  ...
  urgent: { label: 'Time critical', tone: 'danger' },
};
  ...
  return (
    <div>
      ...
      <div className="relative max-w-md">
        <Search
          className="text-ink-quaternary pointer-events-none absolute top-1/2 left-4 h-4 w-4 -translate-y-1/2"
          strokeWidth={2}
        />
        <input
          type="search"
          ...
          className="text-ink placeholder:text-ink-quaternary focus:ring-brand-500 h-12 w-full rounded-[var(--radius-md)] bg-white pr-4 pl-11 text-[0.9375rem] shadow-[var(--shadow-xs)] ring-1 ring-[var(--color-hairline)] transition-shadow ring-inset outline-none focus:ring-2"
        />
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        ...
        {query ? (
          <button
            type="button"
            onClick={() => setQuery('')}
            className="text-ink-quaternary hover:text-ink px-3 py-1.5 text-[0.8125rem] font-medium underline underline-offset-4"
          >
            Clear
          </button>
        ) : null}
      ...
      <div aria-live="polite" className="mt-7">
        {results.length === 0 ? (
          <Card className="bg-white p-8 text-center">
            ...
            <p className="text-ink-tertiary mt-2 text-[0.9375rem]">
              This decoder covers the provisions we see most often. Send us the notice and we
              will read it properly.
            </p>
            <Link
              href="/contact"
              className="text-brand-700 hover:text-brand-800 mt-4 inline-flex items-center gap-1.5 text-[0.9375rem] font-semibold"
            >
              Ask us about it
                  ...
                  <Card className="h-full bg-white p-6">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <p className="text-ink-quaternary mt-1 text-[0.8125rem]">{entry.act}</p>
                      </div>
                    ...
                    <div className="bg-sunken mt-5 rounded-[var(--radius-md)] p-4">
                      <p className="text-ink-quaternary text-[0.75rem] font-semibold tracking-[0.05em] uppercase">
                        What to do
                      </p>
                    ...
                    <p className="text-ink-tertiary mt-4 flex items-center gap-2 text-[0.8125rem]">
                      <Clock className="text-brand-600 h-3.5 w-3.5 shrink-0" strokeWidth={2} />
                      {entry.clock}
                    </p>
                  </Card>
                </li>
