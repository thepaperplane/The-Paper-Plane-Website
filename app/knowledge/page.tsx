    ...
    <>
      ...
      <Section className="py-16 sm:py-20">
        <Container>
          ...
          <ul className="mt-10 grid gap-4 lg:grid-cols-2">
            {MYTHS.map((item, i) => (
              <li key={item.myth}>
                <Card className="h-full bg-white p-6 sm:p-7">
                  <div className="flex items-start gap-4">
                    <span className="bg-sunken text-ink-quaternary flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-[0.8125rem] font-semibold tabular-nums">
                      {i + 1}
                    </span>
                    <div className="min-w-0">
                      <p className="text-ink-tertiary flex items-start gap-2 text-[0.9375rem] font-medium">
                      ...
                      <p className="text-ink mt-3 flex items-start gap-2 text-[0.9375rem] leading-relaxed">
                        <Eye className="text-brand-600 mt-1 h-4 w-4 shrink-0" strokeWidth={2} />
      ...
      <Section tone="sunken" id="exposure" className="scroll-mt-24 py-16 sm:py-20">
        <Container>
          ...
          <Card className="mt-10 overflow-hidden bg-white">
                  ...
                  <tr className="border-b border-[var(--color-hairline)]">
                    {['Trigger', 'Statute', 'Exposure', 'How it compounds'].map((heading) => (
                      <th
                        key={heading}
                        scope="col"
                        className="text-ink-quaternary px-5 py-4 text-[0.75rem] font-semibold tracking-[0.04em] uppercase"
                      >
                        {heading}
                      </th>
                    ))}
                ...
                <tbody className="divide-y divide-[var(--color-hairline)]">
                  {PENALTY_EXPOSURE.map((item) => (
                    <tr key={item.trigger} className="hover:bg-sunken/60 transition-colors">
                      ...
                      <td className="text-ink-tertiary px-5 py-4 align-top text-[0.875rem]">
                        {item.statute}
                      </td>
                      <td className="px-5 py-4 align-top">
                        <span className="text-danger inline-flex items-center gap-1.5 text-[0.9375rem] font-semibold">
                          <TriangleAlert className="h-3.5 w-3.5 shrink-0" strokeWidth={2.2} />
                          {item.exposure}
                        </span>
                      </td>
                      <td className="text-ink-tertiary px-5 py-4 align-top text-[0.875rem] leading-relaxed">
                        {item.compounding}
                      </td>
                    </tr>
                  ))}
          ...
          <p className="text-ink-quaternary mt-4 text-[0.8125rem]">
        ...
        <Container>
          ...
          <div className="mt-10 grid gap-5 sm:grid-cols-2">
            {ARTICLES.map((article) => (
              <Card key={article.slug} interactive className="group bg-white">
                <Link href={`/knowledge/${article.slug}`} className="flex h-full flex-col p-7">
                  ...
                  <h3 className="text-ink group-hover:text-brand-700 mt-4 text-[1.1875rem] leading-snug font-semibold tracking-[-0.015em] transition-colors">
                    {article.title}
                  </h3>

                  <p className="text-ink-tertiary mt-3 flex-1 text-[0.9375rem] leading-relaxed">
                    {article.summary}
                  </p>
                  ...
                  <div className="mt-6 flex items-center justify-between border-t border-[var(--color-hairline)] pt-4">
                    ...
                    <ArrowUpRight className="text-ink-quaternary group-hover:text-brand-600 h-4 w-4 transition-colors" />
                  </div>
                </Link>
              </Card>
            ))}
