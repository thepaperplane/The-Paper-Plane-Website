  ...
  return (
    <Section className="flex min-h-[70dvh] items-center pt-32">
      ...
      <Container size="content" className="text-center">
        ...
        <p className="text-ink-tertiary mx-auto mt-4 max-w-md text-lg leading-relaxed">
          ...
          <ul className="flex flex-wrap justify-center gap-2">
            {PRIMARY_NAV.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className="text-ink-secondary hover:text-ink inline-block rounded-full bg-white px-4 py-2 text-[0.875rem] font-medium shadow-[var(--shadow-xs)] ring-1 ring-[var(--color-hairline)] ring-inset transition-all hover:shadow-[var(--shadow-sm)]"
                >
                  {item.label}
                </Link>
              </li>
